    ...
    x => `
      <template x-teleport="body">
        <div x-show="show" x-transition.opacity class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="show=false">
          <div class="bg-base-100 w-full max-w-3xl max-h-[85vh] shadow-xl rounded-lg flex flex-col m-8" @click.stop>
            ...
            <div class="flex-1 overflow-auto">
              ...
              <div x-show="error" class="text-error text-sm p-3" x-text="error"></div>
              <pre x-show="content && !loading" class="text-xs overflow-auto max-h-[70vh] m-0 p-0"><code x-text="content"></code></pre>
            </div>
          </div>
        </div>
      </template>
